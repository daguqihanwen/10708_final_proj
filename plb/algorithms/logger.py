...
class Logger:
    def __init__(self, path):
        ...
        self.prefix = 'train'
        self.keys = ['step', 'reward', 'info_emd', 'info_normalized_performance', 'info_contact_loss']
    ...
    def step(self, state, action, reward, next_state, done, info):
        ...
        self.values['info_emd'] += info['info_emd']
        self.values['info_normalized_performance'] += info['info_normalized_performance']
        self.values['info_contact_loss'] += info['info_contact_loss']
        ...
        if done:
            ...
            now = datetime.now()
            print(f"{now.strftime('%y-%m-%d %H:%M:%S')} STEP: {self.steps}, reward {self.values['reward']}, info_emd {self.values['info_emd']}, normalized_emd {self.values['info_normalized_performance']},   fps: {fps}")
