...
class Capsule(Primitive):
    def __init__(self, **kwargs):
        ...
        self.h = self.cfg.h
        self.r = self.cfg.r
        self.linsp1 = ti.field(ti.f32, int(np.sqrt(self.num_rand_points)))
        self.linsp2 = ti.field(ti.f32, int(np.sqrt(self.num_rand_points)))

    def initialize(self):
        super(Capsule, self).initialize()
        self._generate_linspace()
    ...
    @ti.kernel
    def _generate_linspace(self):
        n = ti.static(int(ti.sqrt(self.num_rand_points)))
        for i in ti.static(range(n)):
            self.linsp1[i] = -self.h/2 + (self.h *i / (n-1))
            self.linsp2[i] = (2*np.pi*i/(n-1))
    @ti.kernel
...
class RollingPinNew(Capsule):
    # A rollingpin that does not have self-rotation along the rolling direction. (passive rolling)
    @ti.kernel
    def forward_kinematics(self, f: ti.i32):
        vel = self.v[f]
        w = self.w[f]
        ...
        dy = vel[2]  # decrease in y coord...
        y_dir = qrot(self.rotation[f], ti.Vector([0., -1., 0.]))
        x_dir = ti.Vector([0., 1., 0.]).cross(y_dir) * (w[0]) #0.03 radius, so dw*0.03 is roughly rolling length
        x_dir[1] = dy  # direction

        dw = w[0] / self.r  # rotate about object y
        self.rotation[f + 1] = qmul(
            w2quat(ti.Vector([0., -dth, 0.]), self.dtype),
            qmul(self.rotation[f], w2quat(ti.Vector([0., dw, 0.]), self.dtype))
        )
...
class RollingPinExt(Capsule):
    ...
    def forward_kinematics(self, f: ti.i32):
        ...
        dy = vel[2]  # decrease in y coord...
        y_dir = qrot(self.rotation[f], ti.Vector([0., -1., 0.]))
        x_dir = ti.Vector([0., 1., 0.]).cross(y_dir) * (dw * 0.03 + w[0]) #0.03 radius, so dw*0.03 is roughly rolling length
        x_dir[1] = dy  # direction
        self.rotation[f + 1] = qmul(
            w2quat(ti.Vector([0., -dth, 0.]), self.dtype),
            qmul(self.rotation[f], w2quat(ti.Vector([0., dw, 0.]), self.dtype))
    ...
    def forward_kinematics_point(self, states, actions, n_substeps):
        scaled_actions = []
        for i in range(actions.shape[1]):
            scaled_actions.append(actions[:, i] * self.action_scale[None][i] / n_substeps)
        scaled_actions = np.scaled(actions).T
...
class RollingPinExt2(Capsule):
    # A rollingpin whose action space is a SE(3) transformation
    @ti.kernel
    def forward_kinematics(self, f: ti.i32):
